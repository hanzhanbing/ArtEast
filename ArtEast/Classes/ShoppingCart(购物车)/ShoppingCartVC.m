...
- (void)initViews {
    ...
    for (NSInteger i = 0; i < arr.count; i ++) {
        ...
        [first1Btn setTitleEdgeInsets:UIEdgeInsetsMake(0, 0, 0, -5)];
...
- (void)updateDataForUI{
    ...
    for (NSInteger i = 0; i < _dataSource.count; i ++) {
        ...
        cartCount += [model.quantity integerValue];
    }
    if (allID.length>0) {
    ...
    if (cartCount==0) {
        ...
        //显示
        [self.tabBarController.tabBar showBadgeOnItemIndex:3 andCount:[NSString stringWithFormat:@"%d",(int)cartCount]];
    }
    
    [[NSUserDefaults standardUserDefaults] setObject:[NSString stringWithFormat:@"%d",(int)cartCount] forKey:kCartCount];
}
