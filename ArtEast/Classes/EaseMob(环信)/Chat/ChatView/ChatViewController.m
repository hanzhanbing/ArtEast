...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = AEColor(236, 236, 236, 1);
...
- (void)setNavBar {
    ...
    if (self.conversation.type == EMConversationTypeChat) {
        UIBarButtonItem *rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"Delete"] style:UIBarButtonItemStylePlain target:self action:@selector(deleteAllMessages)];
        self.navItem.rightBarButtonItem = rightBarButtonItem;
    }
    else{//群聊
        UIBarButtonItem *rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"group_detail"] style:UIBarButtonItemStylePlain target:self action:@selector(showGroupDetailAction)];
        self.navItem.rightBarButtonItem = rightBarButtonItem;
    }
}
...
- (UITableViewCell *)messageViewController:(UITableView *)tableView
    ...
    TrackOrderCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    cell.userInteractionEnabled = NO; //不可操作
    if (!cell) {
        cell = [[TrackOrderCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
        cell.selectedBackgroundView = [[UIView alloc] initWithFrame:cell.frame];
        cell.selectedBackgroundView.backgroundColor = AEColor(236, 236, 236, 1); //cell选中背景颜色
    }
    
    EMMessage *message = messageModel.message;
    NSLog(@"扩展消息：%@",message.ext);

    if ([messageModel.text isEqualToString:@"商品详情图文混排消息"]) {
        NSDictionary *trackDic = message.ext[@"msgtype"][@"track"];
        NSMutableDictionary *dic = [trackDic mutableCopy];
        [dic setObject:@"轨迹" forKey:@"type"];
        cell.info = dic;
        return cell;
    } else if ([messageModel.text isEqualToString:@"订单图文混排消息"]) {
        NSDictionary *orderDic = message.ext[@"msgtype"][@"order"];
        NSMutableDictionary *dic = [orderDic mutableCopy];
        [dic setObject:@"订单" forKey:@"type"];
        cell.info = dic;
        return cell;
    } else {
        return nil;
                           ...
                           modelForMessage:(EMMessage *)message
{
    ...
    if (model.isSender) {
        ...
        //昵称
        model.nickname = [UserInfo share].userName;
        //头像占位图
