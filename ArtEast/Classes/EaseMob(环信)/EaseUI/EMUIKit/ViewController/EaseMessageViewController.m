...
@interface EaseMessageViewController ()
{
    ...
    NSString *textType; //类型
    NSMutableDictionary *trackDic; //轨迹字典
    NSMutableDictionary *orderDic; //订单字典
}
...
@property(nonatomic,strong) UIView *goodsOrderView; //商品订单视图
...
- (void)viewDidLoad {
    ...
    if (self.info) {
        ...
        //区别自定义消息类型
        textType = @"";
        if ([self.infoType isEqualToString:@"轨迹"]) {
            textType = @"商品详情图文混排消息";
            NSDictionary *dic = self.info[@"msgtype"][@"track"];
            trackDic = [dic mutableCopy];
        }
        if ([self.infoType isEqualToString:@"订单"]) {
            textType = @"订单图文混排消息";
            NSDictionary *dic = self.info[@"msgtype"][@"order"];
            orderDic = [dic mutableCopy];
        }
...
- (void)sendLink {
    EMTextMessageBody *body = [[EMTextMessageBody alloc] initWithText:textType];
    NSString *from = [[EMClient sharedClient] currentUsername];
    
    //生成Message
    EMMessage *message = [[EMMessage alloc] initWithConversationID:self.conversation.conversationId from:from to:self.conversation.conversationId body:body ext:self.info];
    message.chatType = EMChatTypeChat;// 设置为单聊消息
    message.ext = self.info; // 扩展消息部分
    [self _sendMessage:message];
    
    NSLog(@"扩展消息字典：%@",message.ext);
}
...
//        _timeLab.text = [NSString stringWithFormat:@"今天 %@",dateString];
//        _timeLab.font = [UIFont systemFontOfSize:12];
//        _timeLab.textColor = [UIColor grayColor];
//        _timeLab.textAlignment = NSTextAlignmentCenter;
//        [_headerView addSubview:_timeLab];
        
        _goodsOrderView = [[UIView alloc] initWithFrame:CGRectMake(0, 5, WIDTH, 90)];
        _goodsOrderView.backgroundColor = [UIColor whiteColor];
        [_headerView addSubview:_goodsOrderView];
        
        _imageView = [[UIImageView alloc] initWithFrame:CGRectMake(10, 10, 70, 70)];
        [_goodsOrderView addSubview:_imageView];
        
        _nameLab = [[UILabel alloc] initWithFrame:CGRectMake(_imageView.maxX+10, 10, WIDTH-105, 20)];
        _nameLab.numberOfLines = 0;
        _nameLab.textColor = LightBlackColor;
        _nameLab.font = [UIFont systemFontOfSize:14];
        [_goodsOrderView addSubview:_nameLab];
        
        _priceLab = [[UILabel alloc] initWithFrame:CGRectMake(_imageView.maxX+10, _imageView.maxY-15, WIDTH-200, 15)];
        _priceLab.textColor = PriceRedColor;
        _priceLab.font = [UIFont systemFontOfSize:14];
        [_goodsOrderView addSubview:_priceLab];
        
        _sendBtn = [[UIButton alloc] initWithFrame:CGRectMake(WIDTH-100, _imageView.maxY-30, 90, 30)];
        _sendBtn.layer.cornerRadius = 3;
        _sendBtn.layer.borderColor = AppThemeColor.CGColor;
        _sendBtn.layer.borderWidth = 1;
        [_sendBtn setTitleColor:AppThemeColor forState:UIControlStateNormal];
        _sendBtn.titleLabel.font = [UIFont systemFontOfSize:13];
        [_sendBtn addTarget:self action:@selector(sendLink) forControlEvents:UIControlEventTouchUpInside];
        [_goodsOrderView addSubview:_sendBtn];
        
        if ([self.infoType isEqualToString:@"轨迹"]) {
            [_sendBtn setTitle:@"发送商品链接" forState:UIControlStateNormal];
            [_imageView sd_setImageWithURL:[NSURL URLWithString:trackDic[@"img_url"]] placeholderImage:[UIImage imageNamed:@"GoodsDefault"]];
            _nameLab.text = trackDic[@"desc"];
            _priceLab.text = trackDic[@"price"];
            
            CGSize nameSize = [trackDic[@"desc"] boundingRectWithSize:CGSizeMake(WIDTH-105,CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:14],NSFontAttributeName,nil] context:nil].size;
            _nameLab.height = nameSize.height;
        }
        if ([self.infoType isEqualToString:@"订单"]) {
            [_sendBtn setTitle:@"发送订单链接" forState:UIControlStateNormal];
            [_imageView sd_setImageWithURL:[NSURL URLWithString:orderDic[@"img_url"]] placeholderImage:[UIImage imageNamed:@"GoodsDefault"]];
            _nameLab.text = orderDic[@"desc"];
            _priceLab.text = orderDic[@"price"];
            
            CGSize nameSize = [orderDic[@"desc"] boundingRectWithSize:CGSizeMake(WIDTH-105,CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:14],NSFontAttributeName,nil] context:nil].size;
            _nameLab.height = nameSize.height;
        }
    }
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:didLongPressRowAtIndexPath:)]) {
            ...
            if (![object isKindOfClass:[NSString class]]) {
                ...
                if ([cell isKindOfClass:[EaseMessageCell class]]) {
                    ...
                    [emcell becomeFirstResponder];
                    _menuIndexPath = indexPath;
                    [self showMenuViewController:emcell.bubbleView andIndexPath:indexPath messageType:emcell.model.bodyType];
                }
            }
        }
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(messageViewController:cellForMessageModel:)]) { //respondsToSelector：.m是否实现方法
...
{
    NSDictionary *dataDictionary = @{@"trueName":[UserInfo share].userName,
                                     @"qq":@"",
                                     @"companyName":@"",
                                     @"userNickname":[UserInfo share].userName,
                                     @"description":@"",
                                     @"email":@"",
                                     @"avatar":[UserInfo share].avatar
                                     };
    NSDictionary *ext = @{@"weichat":@{@"visitor":dataDictionary}};
    [self sendTextMessage:text withExt:ext];
}
