...
{
    //avatar view
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.avatarView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeTop multiplier:1.0 constant:13]];
...
- (void)setModel:(id<IMessageModel>)model
{
    ...
    NSLog(@"聊天头像  %@",model.avatarURLPath);
    
    if (self.model.isSender) { //是否是当前登录者发送的消息
        _hasRead.hidden = YES;
        ...
        [self.avatarView sd_setImageWithURL:[NSURL URLWithString:[UserInfo share].avatar] placeholderImage:[UIImage imageNamed:@"MineHeadIcon"]];
//        _nameLabel.text = [UserInfo share].userName;
    }else{
        [self.avatarView sd_setImageWithURL:[NSURL URLWithString:@"http://images-app.cydf.com/public/120-1.png"] placeholderImage:[UIImage imageNamed:@"MineHeadIcon"]];
//        _nameLabel.text = @"小佑儿";
    }
}
