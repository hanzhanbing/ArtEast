...
@interface BaseWebVC ()<WKUIDelegate,WKNavigationDelegate,WKDelegate,RefreshDelegate,UISearchBarDelegate,UIGestureRecognizerDelegate>
...
- (void)isPinCodeWithImage:(UIImage *)image{
    ...
    for(symbol in  [read scanImage:cgImageRef])
        break;
    if ([symbol.data hasPrefix:@"http"]) { //图片是二维码
        //弹框提示
